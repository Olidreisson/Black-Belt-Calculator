...
document.getElementById("info-button").addEventListener("click", () => {
  ...
  document.getElementById("calculate-button").addEventListener("click", function () {
    ...
    const totalLoggedHours = Math.min(elapsedYears * frequency * hoursPerTraining * 52, totalYears * frequency * hoursPerTraining * 52);
  
    const remainingHours = totalYears * frequency * hoursPerTraining * 52 - totalLoggedHours;
  
    const progressPercentage = Math.min((totalLoggedHours / (totalYears * frequency * hoursPerTraining * 52)) * 100, 100);
  
    // Atualizar resultados
    document.getElementById("time-to-black-belt").innerText = `Faltam aproximadamente ${(remainingHours / (frequency * hoursPerTraining * 52)).toFixed(1)} anos.`;
